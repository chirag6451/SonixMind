...
# VoiceFixer Model Files

This directory is used to store VoiceFixer model checkpoints.

## Required Files

To use the VoiceFixer functionality in SonixMind, you need to download the following model files from the [VoiceFixer repository](https://github.com/haoheliu/voicefixer):

1. Download the pre-trained model files
2. Place them in this directory

## Directory Structure

The expected structure after downloading is:
```
voicefixer/
├── model_1.pt
├── model_2.pt
...
Note: These model files are excluded from the Git repository due to their large size. Each user needs to download them separately.

## Troubleshooting

If you encounter issues with voice fixing functionality, ensure that:
1. All required model files are downloaded
2. Files are placed in the correct locations as specified above
3. File permissions allow the application to read these files
